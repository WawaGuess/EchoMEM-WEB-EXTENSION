...
把 EchoMem Episode 展示为独立的“情节记忆”视图，让用户先理解一段经历是什么、如何延续、包含哪些关键事件，再按需进入详情查看完整信息。

## 信息架构

本功能注册到认知反馈公共壳，只提供“情节记忆”视图：

- **情节记忆**：展示 Episode 时间范围、连续性、事件、标签与可用操作。

默认进入情节记忆。视图切换时销毁前一个视图的事件绑定和异步状态，避免迟到请求覆盖当前页面。

...
会话录制模块负责监听支持平台（DeepSeek、HIGO）的聊天页面 DOM 变化，从页面中提取用户与 AI 的消息记录，并同步到 EchoMem 记忆后端引擎。
...
- **模块关系**：
  ...
  - `session-recorder.js`：状态管理层，负责 MutationObserver 监听、diff 计算、流式检测、EchoMem 同步。

## 2. 核心文件

| 文件 | 职责 |
|------|------|
| `src/core/session-recorder.js` | MutationObserver 挂载、消息 diff、流式状态机编排、EchoMem HTTP 同步。**平台无关**，所有差异通过 adapter / 配置注入 |
...
| `src/services/echomem-client.js` | `EchoMemClient`：封装 `openSession`、`addMessage`、`appendMessages`、`commitSession` |
...
| `src/config/loader.js` | 加载声明式平台 JSON 配置，按通用规则工作、不写平台 id 分支 |
...
- **初始基线**：
  - 如果已有 EchoMem session 映射（从 `chrome.storage.local` 恢复），将当前 DOM 消息设为 `lastMessages` 基线，跳过已发送消息
...
- 如果尚未创建 EchoMem session，先调用 `openSession`
- 如果发送失败，将消息重新压入队列
...
└─────────────────┘                   └────────┬────────┘
           ...
           └────────┬────────┘      └────────┬────────┘
                    ...
                    ▼                        ▼
           ┌─────────────────┐      ┌─────────────────┐
           │ EchoMem POST     │      │ EchoMem POST     │
           │ (openSession     │      │ (appendMessages) │
           │  + addMessage)   │      │                  │
           └─────────────────┘      └─────────────────┘
```
...
多轮对话后 EchoMem 中仍偶发重复消息（如 `user2, assistant2, user2, assistant2`）。重复可能在以下情况发生：
- `sendStreamingResult` 与 `onMessagesChanged` 的非流式 diff 产生竞态
- EchoMem 服务端或网络层重试导致重复入库
...
3. 增加 EchoMem 侧幂等性校验（如 message sequence number）

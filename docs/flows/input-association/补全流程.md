...
**核心逻辑**（`commitSelectedSuggestions` → `composeAndInsert`）：
...
function composeAndInsert(inputElement, userText, selected) {
  ...
  // 4. 统一写入 textarea/input 或 contenteditable，并触发 input 事件
  return setEditableText(inputElement, next, { cursor: next.length });
}
...
**`stripMemoryBlock`**：删除已有的 `<relevant-memories>...</relevant-memories>` 标签块并保留用户原文；检测到旧记忆块时同步清空 `committedItems`，随后按本次选中内容重建记忆段。
...
| 配置项 | 默认值 | 范围 | 说明 |
|--------|--------|------|------|
| `baseUrl` | 公网版由 `ECHOMEM_PUBLIC_BASE_URL` 注入；内网版由 `ECHOMEM_INTRANET_BASE_URL` 注入 | - | EchoMem 服务地址；用户已保存值优先 |
...
- **卡片内操作反馈**：测试连接与保存配置共用「记忆后端引擎」卡片的状态区；EchoAgent 登录使用对应连接卡片内的独立状态区。进行中、成功、失败以及配置修改后的状态均就近保持可见，不再向宿主页面挂载全局悬浮 toast。
- **输入联想高级配置反馈**：短语过滤阈值的保存过程与结果显示在「补全算法配置」卡片内；修改阈值后提示重新保存，不再使用宿主页面全局 toast。
- **异步状态保护**：操作进行中禁用卡片内的冲突按钮；输入内容变化后提示重新测试、保存或登录，并通过 revision 校验忽略旧配置产生的过期异步结果。
#### 6.3 加载与保存
...
用户勾选第 1、3 条记忆，点击「确定 (2)」
    │
    ▼
commitSelectedSuggestions(inputElement, [
  { key: 'viking://.../mem1', item: completion1 },
  { key: 'viking://.../mem3', item: completion3 }
])
    │
    ├── readEditableText(inputElement)
    │   └── textarea/input 读取 value；contenteditable 读取保留换行的 innerText
    │
    ├── stripMemoryBlock(当前文本)
    │   ├── 查找 <relevant-memories>...</relevant-memories>
    │   ├── 若存在：删除旧记忆块，返回用户原文
    │   └── 若不存在：返回原文
    │
    ...
    │   └── "用户原文\n\n<relevant-memories>\n1. TypeScript 类型系统优势\n2. Docker 容器化部署\n</relevant-memories>"
    │
    └── setEditableText(inputElement, 新文本, { cursor: 新文本.length })
        ├── 兼容 textarea/input 和 contenteditable
        ├── 设置光标到末尾
        ├── 触发冒泡 input 事件
        └── 聚焦输入框
```

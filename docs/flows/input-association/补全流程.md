...
- **连接测试反馈**：测试中、成功、失败以及测试后修改配置的状态，均显示在「记忆后端引擎」卡片的操作按钮下方；测试期间禁用重复提交，配置发生变化后提示重新测试。异步测试结果带 revision 校验，避免旧配置的响应覆盖新配置状态。
- **其他操作反馈**：保存配置与 OpenView 登录仍通过全局悬浮 toast 提示（`src/services/toast.js`）。
#### 6.3 加载与保存

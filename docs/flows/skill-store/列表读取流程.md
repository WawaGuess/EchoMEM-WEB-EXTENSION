...
- **Skill 卡片**：上层使用完整宽度展示名称和描述，下层集中展示版本/时间与「使用」「详情」操作，避免长名称被操作区挤压
- **直接使用**：「我的 Skill」点击卡片或通过键盘触发语义化「使用」按钮后，将 `/dirName` 插入聊天输入框，关闭面板并聚焦输入框，不自动发送

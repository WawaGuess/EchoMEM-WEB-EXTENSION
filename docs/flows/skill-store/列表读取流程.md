...
- `.zip` Skill Package：限制 45 MB，读取 `ArrayBuffer` 并转换为 Base64，再调用 `client.addSkillPackage({ packageBase64, filename })`；编码后约 60 MiB，为单条扩展消息的 JSON 包装预留空间；该请求默认超时 120 秒

...
用户点击「详情」
    │
    ▼
切换到独立详情页（description + 正文预览 + 查看完整内容 + 版本历史 + URI）
```
...
- **Skill 卡片**：上层使用完整宽度展示名称和描述，下层集中展示版本/时间与「点击使用」「详情」操作，避免长名称被操作区挤压
- **直接使用**：「我的 Skill」点击卡片后将 `/dirName` 插入聊天输入框，关闭面板并聚焦输入框，不自动发送
- **独立详情页**：点击「详情」后从列表切换到详情页，显示 `description`、正文预览、「查看完整内容」、URI，并懒加载版本历史

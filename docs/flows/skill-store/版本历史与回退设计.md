...
> 相关代码：`src/services/echomem-client.js`、`src/panels/skill-store/index.js`、`src/panels/skill-store/version-history.js`
...
2. 用户首次展开某个 Skill 时，懒加载该 Skill 的版本历史。
...
这样可以保持两个入口职责清晰，并避免版本回退按钮与删除按钮在同一卡片中竞争操作层级。
...
同一 Skill 重复展开时不重复请求版本列表；只有手动刷新、上传覆盖、删除或回退成功时才失效相关缓存。
...
- 请求路径、缓存、请求去重、回退中状态和重新展开都使用 `apiName`；卡片、Toast 和确认文案仍使用 frontmatter 展示名。
...
| 编号 | 验收项 |
|------|--------|
| 1 | 「我的 Skill」现有列表、搜索、展开和查看完整内容功能保持可用 |
| 2 | 首次展开 Skill 时才请求 `/api/skills/{name}/versions`，列表首屏不增加版本请求 |

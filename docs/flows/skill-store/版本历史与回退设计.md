...
这样可以保持两个入口职责清晰，并避免版本回退按钮与删除按钮在同一卡片中竞争操作层级。「我的 Skill」卡片主体只负责使用 Skill，卡片内独立「详情」按钮负责展开、收起和版本历史懒加载。

### 5.2 详情布局

点击独立「详情」按钮后，详情区域按以下顺序展示：
...
| 13 | 所有新增服务端文本经过安全转义 |
| 14 | 「详情」按钮独立承载展开、完整内容、版本历史和回退，不与卡片使用操作冲突 |

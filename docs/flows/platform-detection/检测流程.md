...
EchoMem 扩展按平台执行已配置的主机、路径、标题、DOM 和内容检测。HIGO 额外将主机分为官方可信主机与 localhost/IP 回退主机：官方主机不等待 DOM，回退主机使用“品牌信号 + 任一语义 DOM 特征”二次确认。不要求“工作空间图标 + 标题栏菜单”或“输入框 + 发送按钮”这类固定组合。目前已支持 HIGO Office 和 DeepSeek 两个平台。
...
| 可信主机 | `echo-agent.online`, `www.echo-agent.online`, `higo.world` |
...
HIGO 官方域名不配置全局标题硬门槛。只有 localhost/IP 回退主机使用 `titleOrContentKeywords`，且标题或正文命中任一品牌词即可。

...
EchoMem 扩展支持最多 4 层检测机制，确保只在目标网站的特定页面注入增强功能。每个平台只执行已配置的检测层，这些层全部满足才判定为目标平台。目前已支持 HIGO Office 和 DeepSeek 两个平台。
...
HIGO 不配置这一层。页面标题可能在 React 首屏渲染期间延迟更新，不作为平台识别的硬门槛。

### 第三层：DOM 特征检测

#### 必要元素（required）

HIGO 不配置必要 DOM 元素。`.MuiDrawer-root` 和 `.MuiPaper-root` 属于宿主实现细节，不再作为硬门槛。
...
通用 MUI 容器和右侧抽屉不再用于识别平台。首次直接访问 `/home/workspace` 时，MutationObserver 会在工作空间导航图标完成动态渲染后重试检测和注入。

### 第四层：页面内容关键字

HIGO 不配置这一层。可见文案会受加载时机、品牌配置和国际化影响，不作为平台识别的硬门槛。
...
```
Claw Extension: 平台检测未通过 - 无可选DOM特征匹配
```

### 检测成功示例

```
Claw Extension: 平台检测全部通过: ✓ 主机匹配 | ✓ 路径匹配 | ✓ 可选DOM特征匹配

...
4. 功能导航首页展示 5 个选项：资源管理、输入联想、认知反馈、Skill 管理、效能。
...
采用 **Organic Liquid（有机液态）** 设计风格：

- **背景**：暖色渐变 `#f5f0eb → #ede7e0`，与 HIGO 页面自然融合
- **卡片**：毛玻璃效果 `backdrop-filter: blur(12px)`，半透明白色背景
- **图标**：水滴形圆角容器（`border-radius: 50% 50% 50% 50% / 60% 60% 40% 40%`），大地色系渐变背景，内部放置白色功能 SVG 图标
...
```
┌──────────────────────────────────┐
│  EchoMem                    ×    │  ← 标题 22px 加粗，关闭按钮圆形
├──────────────────────────────────┤
│                                  │
│  ◯ 资源管理                 >    │  ← 渐变图标容器 + 标题/描述 + 箭头
│       上传、查看和管理知识资源    │     毛玻璃卡片，hover 上浮阴影
│                                  │
│  ◯ 输入关联                 >    │
│       开启后自动关联相关内容      │
│                                  │
...
│                                  │
│  ─────────────────────────────   │
│  ◯ 记忆后端引擎连接配置      >    │  ← 配置项分隔线，margin-top: 8px
│       配置后端地址和 API Key      │
│                                  │
└──────────────────────────────────┘
```
...
面板标题和描述从 `src/panels/registry.js` 中读取，颜色通过 `panelId` 映射到固定的 `panelStyles` 配置。
...
5. 5 个选项分别为：资源管理、输入联想、认知反馈、Skill 管理、效能。
